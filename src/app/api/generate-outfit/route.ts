...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `[generate-outfit] Received — body: ${bodyPhoto.substring(0, 40)}... (${(bodyPhoto.length / 1024).toFixed(0)}KB), garment: ${clothingItem.substring(0, 40)}... (${(clothingItem.length / 1024).toFixed(0)}KB)`
    );
    ...
    console.log(
      `[generate-outfit] Prepared inputs — human: ${typeof humanImg === "string" ? "URL" : "File"}, garment: ${typeof garmImg === "string" ? "URL" : "File"}`
    );

    console.log("[generate-outfit] Calling Replicate IDM-VTON...");
    ...
    console.log(
      "[generate-outfit] Raw output type:",
      typeof output,
    ...
    console.log("[generate-outfit] Resolved imageUrl:", imageUrl?.substring(0, 150) || "NONE");
